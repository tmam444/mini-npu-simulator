...
### 1. 실패 원인 분석 및 모드별 동점 처리 정책 차이
이 시뮬레이터는 부동소수점 오차 방어를 위해 두 점수의 차이가 `1e-9` 미만일 때 `UNDECIDED`(동점) 판정을 내립니다. 이 정책은 **실행 모드의 목적(실시간 UI vs 배치 검사)**에 따라 다르게 해석됩니다.

- **모드 1 (실시간 UI)**: 인간 사용자가 직접 확인하는 모드이므로, 단순히 `UNDECIDED` 상태임을 화면에 출력하여 "판정할 수 없음"을 안내합니다.
- **모드 2 (배치 검사)**: 자동화된 정답 대조 모드입니다. 정답(`expected`)은 항상 명확하게 존재해야 하므로, 시뮬레이터가 확정적인 답을 내지 못하고 `UNDECIDED`를 반환하면 무조건 **`FAIL (동점 규칙)`**로 엄격하게 집계합니다.

모드 2(`data.json` 분석) 실행 시 발생하는 `FAIL` 케이스의 주된 원인은 다음과 같습니다:
* **동점 규칙 (UNDECIDED)**: 의도적으로 헷갈리게 설계된 데이터에서 필터 점수가 완벽히 동일할 때 발생 (모드 2 정책에 따라 일괄 FAIL 처리)
...
> 💡 **시사점:** 크기가 커질수록 연산량이 기하급수적으로 증가합니다. 딥러닝 현업에서 CPU 대신 극도의 병렬 처리가 가능한 **전용 하드웨어(NPU) 시스톨릭 어레이**를 도입하는 근본적인 이유가 여기에 있습니다. 특히 1000×1000 이상의 대규모 행렬 연산에서는 단순히 연산기를 늘리는 것을 넘어 메모리 병목 현상을 해결하기 위해 메모리 대역폭 절감(가중치 재사용), 캐시 친화적인 스트라이드(Stride) 접근, 타일링(Blocking) 등의 소프트웨어/하드웨어적 최적화가 필수적입니다.
