...
NPU는 **MAC 연산 하나만을 극도로 효율적으로 수행**하기 위해 설계된 칩입니다.
...
│  ┌──────────┐   │  │ PE │ PE │ PE │ PE │ PE │ PE │  │    │
│  │ 온칩 SRAM │   │  ├────┼────┼────┼────┼────┼────┤  │    │
│  │ (수 MB)   │   │  │ PE │ PE │ PE │ PE │ PE │ PE │  │    │
│  │           │──▶│  └────┴────┴────┴────┴────┴────┘  │    │
│  └──────────┘   │     PE = Processing Element        │    │
│                  │     (곱셈기 1개 + 덧셈기 1개)       │    │
│  ┌──────────┐   └──────────────────────────────────┘    │
│  │ DMA 엔진 │   데이터를 DRAM ↔ SRAM 간 전송            │
│  └──────────┘                                           │
└─────────────────────────────────────────────────────────┘
```
...
정밀한 과학 계산이 필요한 경우, NPU의 양자화가 **허용할 수 없는 오차**를 만들 수 있습니다.
...
#### 기법 2: 양자화 (Quantization)

모델의 가중치와 입력을 낮은 비트로 변환하여 연산 속도를 높입니다:

```
FP32 모델 (기본):
  가중치: 0.37842193 (32비트 = 4바이트)
  1회 곱셈 = 32비트 곱셈기 사용

INT8 양자화 후:
  가중치: 97 (8비트 = 1바이트, 0.37842 × 255 ≈ 97)
  1회 곱셈 = 8비트 곱셈기 사용

효과:
  - 메모리 4배 절감 (4바이트 → 1바이트)
  - 연산 속도 2~4배 향상
  - 정확도 손실: ~1~2% (대부분의 AI 작업에서 허용 범위)
```
...
#### 기법 4: 가중치 재사용 (Weight Reuse)

Systolic Array의 핵심 장점은 **한 번 로드한 가중치를 여러 입력에 재사용**하는 것:

```
필터(가중치)를 PE 배열에 고정 → 입력 데이터만 흘려보냄

  고정된 가중치 w₁, w₂, w₃...
       ↓   ↓   ↓
입력 → PE → PE → PE → 결과
x₁  → w₁x₁  w₂x₁  w₃x₁
x₂  → w₁x₂  w₂x₂  w₃x₂   ← 가중치를 다시 로드할 필요 없음!
x₃  → w₁x₃  w₂x₃  w₃x₃

→ 메모리 접근 횟수 대폭 감소 → 전력 절감 + 속도 향상
```
...
1. 이미지 캡처 (12MP = 4000×3000 픽셀)
2. NPU에서 동시에 실행:
   ├── 노이즈 제거 (3×3~5×5 필터 수천 개 적용)
   ├── HDR 합성 (여러 노출 이미지의 행렬 합성)
   ├── 얼굴 감지 (컨볼루션 필터로 특징 추출)
   ├── 피사체 분리 (세그멘테이션)
   └── 색상 보정

소요 시간: ~25ms (초당 40프레임 실시간 처리)
CPU로 같은 작업: ~2,000ms (50배 이상 느림)

MAC 연산 횟수: 약 100억 회/프레임
NPU(38 TOPS)가 처리: 약 0.26ms면 100억 MAC 가능
```
...
음성 파형 → FFT(주파수 변환) → 멜 스펙트로그램(2D 행렬)
    ↓
NPU에서 Transformer 모델 실행:
    ├── Attention 행렬: (토큰 수 × 토큰 수) 크기의 행렬곱
    ├── FFN 레이어: (토큰 수 × 은닉 차원) 행렬곱
    └── 출력 확률 계산: Softmax 연산

예시 (Whisper 모델):
  입력: 30초 음성 → 1500×80 멜 스펙트로그램
  모델: ~1.5B 파라미터
  필요 MAC: ~30억 회
  NPU 처리: ~0.5초 (실시간보다 빠름)
  CPU 처리: ~15초 (실시간보다 느림)
```
...
```
[ChatGPT 같은 LLM 한 글자 생성에 필요한 연산]

GPT-4 급 모델 (추정):
  파라미터 수: ~1.8조 개
  1 토큰 생성 시: ~3.6조 번의 MAC 연산

NVIDIA B200 (1,979 TOPS):
  3.6조 ÷ 1,979조 = ~1.8ms/토큰
  → 초당 ~550 토큰 생성 가능

일반 CPU (Intel i9, ~1 TOPS):
  3.6조 ÷ 1조 = ~3,600ms/토큰
  → 초당 ~0.28 토큰 (사실상 사용 불가)
```
...
카메라 8대 × 30프레임 = 초당 240장의 이미지

각 이미지에 대해:
  ├── 객체 인식: 차, 사람, 신호등 등 감지 (YOLOv8)
  ├── 차선 인식: 도로 경계 감지
  ├── 깊이 추정: 2D → 3D 거리 추정
  └── 예측: 다른 차량의 경로 예측

총 MAC 연산: ~100조 회/초
Tesla FSD Chip (72 TOPS × 2): 144 TOPS로 처리

지연 시간 제약: 100ms 이내 (인간 반응 시간의 1/3)
→ CPU로는 물리적으로 불가능한 작업량
```
